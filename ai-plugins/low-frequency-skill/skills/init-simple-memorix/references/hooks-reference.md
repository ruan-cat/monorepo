...
| ZCode       | `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PostToolUseFailure` | 不放入精简生命周期模板，避免工具与权限请求噪音。 |
| Antigravity | `PreToolUse`, `PostToolUse`                                            | 移除，尤其是 `matcher: "*"` 的全局高频配置。     |

不要把 `UserPromptSubmit`、`beforeSubmitPrompt`、`PreCompact`、`preCompact` 误判为噪音；它们在支持这些事件的平台上是低频生命周期节点。ZCode 配置文件 hooks 不支持 `PreCompact`，不要为了补齐压缩前生命周期而创建不存在的事件。

## 生命周期 hooks

| 事件类型     | Claude Code        | Codex              | Cursor               | Windsurf                | Gemini CLI     | WorkBuddy          | ZCode              | Antigravity              |
| :----------- | :----------------- | :----------------- | :------------------- | :---------------------- | :------------- | :----------------- | :----------------- | :----------------------- |
| 会话开始     | `SessionStart`     | `SessionStart`     | `sessionStart`       | `pre_user_prompt`       | `SessionStart` | `SessionStart`     | `SessionStart`     | `PreInvocation`          |
| 用户提交     | `UserPromptSubmit` | `UserPromptSubmit` | `beforeSubmitPrompt` | -                       | -              | `UserPromptSubmit` | `UserPromptSubmit` | -                        |
| 压缩前       | `PreCompact`       | -                  | `preCompact`         | -                       | `PreCompress`  | `PreCompact`       | -                  | -                        |
| 会话结束     | `Stop`             | `Stop`             | `stop`               | `post_cascade_response` | -              | `Stop`             | `Stop`             | `PostInvocation`, `Stop` |
| Agent 响应后 | -                  | -                  | -                    | -                       | `AfterAgent`   | -                  | -                  | -                        |

Codex 和 ZCode 都缺少 `PreCompact`；不要为了“补齐四件套”创造不存在的事件。ZCode 配置文件当前支持的事件集合是 `SessionStart`、`UserPromptSubmit`、`PreToolUse`、`PermissionRequest`、`PostToolUse`、`PostToolUseFailure`、`Stop`。
...
| WorkBuddy   | 与 Claude Code 事件名一致，但项目级 hooks 仍是实验性、待运行时验证；每个事件数组内是 `{ "matcher": "*", "hooks": [...] }` 嵌套结构，hook 条目带 `description`。                                                                                                 |
| ZCode       | 配置文件 hooks 结构为 top-level `hooks.enabled: true`、`hooks.timeoutMs` 和 `hooks.events`；每个事件数组内使用 `{ "matcher": ".*", "hooks": [...] }`，hook 命令为 `memorix.cmd hook`。`matcher` 是大小写敏感正则，可省略；不能写 `*`，需要匹配全部时使用 `.*`。 |
...
ZCode 项目级模板是 `templates/zcode.config.json`，目标形态用于项目根目录 `.zcode/config.json` 或 `zcode.json`。ZCode 配置文件 hooks 默认禁用，必须显式设置 `hooks.enabled: true`；事件必须写在 `hooks.events` 下，不能写成插件 hooks 顶层事件结构。精简 lifecycle 模板只保留 `SessionStart`、`UserPromptSubmit`、`Stop`，不要包含 `PreCompact`。
...
- WorkBuddy 要保留 `matcher`、`description` 等结构字段，不能扁平化；项目级 hooks 目标和模板仍标注为实验性、待运行时验证。
- ZCode 要保留 `hooks.enabled: true` 和 `hooks.events` 层级，不能把 `SessionStart` 等事件直接放在 `hooks` 下。
- ZCode 不支持 `PreCompact`，合并已有配置时不要新增这个事件。
...
- WorkBuddy 仍是 matcher 嵌套结构，项目级模板为实验性、待运行时验证。
- ZCode 项目级模板存在于 `templates/zcode.config.json`，目标路径是 `.zcode/config.json` 或 `zcode.json`。
- ZCode 使用配置文件 hooks 结构：`hooks.enabled: true` 且事件在 `hooks.events` 下。
- ZCode lifecycle 模板只包含 `SessionStart`、`UserPromptSubmit`、`Stop`，不包含不支持的 `PreCompact`。
