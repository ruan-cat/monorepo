...
- 版本号与 marketplace 主版本同步至 `6.9.0`（本次 `dev-skills` 技能树无内容变更；`common-tools` 的 **init-ai-md** 新增 Karpathy 编码行为准则模板）。

**v6.8.0 openspec 动态任务补全**:

- `openspec` 升级到 `1.3.0`，新增执行中动态补全 `tasks.md` 的规则。
- 发现遗漏任务时先回写当前 change 的 `tasks.md`，再继续实现或验收。
- 动态任务改变用户可观察行为时，同步 specs 并执行 strict validate。
- 版本号与 marketplace 主版本同步至 `6.8.0`。

⚠️ **v6.7.0 版本同步**:

- 版本号与 marketplace 主版本同步至 `6.7.0`（本次 `dev-skills` 技能树无内容变更；`common-tools` 增强 **pr-ruancat-repo** 的批量 PR 策略）。

⚠️ **v6.6.0 版本同步**:

- 版本号与 marketplace 主版本同步至 `6.6.0`（本次 `dev-skills` 技能树无内容变更；`common-tools` 新增 **do-long-task** 长任务执行纪律技能）。
...
执行中发现任务遗漏时，必须先回写当前 change 的 `tasks.md` 并运行 strict validate，再继续实现或验收。
