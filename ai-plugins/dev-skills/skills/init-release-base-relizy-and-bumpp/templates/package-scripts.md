...
| `release:dry`         | relizy dry-run 模式，用于测试；缺 baseline tags 时只提示兜底命令，不写 tag |
...
4. 首次接入 independent 且缺 baseline tags 时，`release:dry` 可能因禁写保护提前停止；真实 `release:sub --no-push` 才会自动创建本地 annotated baseline tags，并由后续 `git push --follow-tags` 携带。
