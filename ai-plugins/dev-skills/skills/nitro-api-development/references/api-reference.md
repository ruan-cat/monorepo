...
> 本节是字段速查，不是可直接复制的完整配置。纯 Nitro API 项目必须以 [SKILL.md](../SKILL.md) 的 10.3 节为唯一完整配置起点；其中的 `pkg.version`、OpenAPI JSON 与 Scalar 页面不可省略。Vite 全栈项目按 10.4 集成。
