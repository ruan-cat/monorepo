...
2. **常用命令**（与实际 `package.json` 一致）：
   ...
   - `pnpm run release` — 发版（若使用 runner，写明实际命令）。
...
5. **兼容说明**：若存在 runner 或 patch，说明**为何需要**、**不负责**改变 relizy 语义，仅解决平台/调用链问题。
6. **与 CHANGELOG 的关系**：根 `CHANGELOG.md` 与子包 changelog 的生成规则；README 不替代 changelog 文件。
