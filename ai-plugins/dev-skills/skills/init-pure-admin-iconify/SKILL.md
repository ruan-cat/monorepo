...
完整的 API 参考、渐变注入原理、三层动画策略选择、CSS 方案、多卡片场景、全局协调器、已知约束与坑位等详细内容，见以下 reference 文件：

| reference 文件                     | 内容                                                                                                   | 何时阅读                     |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ---------------------------- |
| `references/useIconEffect.md`      | 完整 API 参考、渐变注入核心原理、三层动画策略（GSAP/CSS/Auto）、CSS 方案、策略选择指南、已知约束与坑位 | 需要使用 useIconEffect 时    |
| `references/breath-coordinator.md` | 全局协调器模式（Pinia/Composable）、按实例独立使用的代价量化、决策树、与 KeepAlive 配合                | ≥4 实例或需要全局暂停/恢复时 |

### 模板文件

| 模板文件                                    | 说明                                          |
| ------------------------------------------- | --------------------------------------------- |
| `templates/ReIcon/src/useIconEffect.ts`     | useIconEffect composable 源码                 |
| `templates/ReIcon/type.ts`                  | 类型定义（GradientStop、IconBreathConfig 等） |
| `templates/breathCoordinator.pinia.ts`      | 全局协调器 — Pinia Store 版本                 |
| `templates/breathCoordinator.composable.ts` | 全局协调器 — 纯 Composable 版本               |
