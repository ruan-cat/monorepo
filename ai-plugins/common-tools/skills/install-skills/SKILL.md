...
- 默认且唯一的同步策略是符号链接（目录级链接）；禁止把规范源目录复制为实体副本。全机只允许存在一份物理副本。
...
- 预期形态说明：目录级链接建立后，平台 skills 目录本身就是一条链接；浏览时看到的条目是规范源目录的真实条目，属于链接穿透的正常现象，不是实体拷贝，也不会产生逐条快捷方式箭头。判断是否误复制，以链接目标处写入文件能否即时出现在平台目录为准。
