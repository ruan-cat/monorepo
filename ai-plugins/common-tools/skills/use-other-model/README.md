...
本次升级在既有 A-D 调用路径之上增加模型分层：主代理负责规划、复杂诊断和最终验收，弱/中模型负责边界明确的执行，独立强模型负责蓝军复核与安全审计。
...
- **模型分层**：主代理负责规划/诊断/验收，弱中模型负责边界明确的执行，强模型独立做蓝军与安全审计
...
1. 先按模型分层策略确定角色：主代理、执行型、诊断协作者或审计型；再选方案 A、方案 B、方案 C 还是方案 D。
...
| ------------ | -------- | ---------------------------------- |
| 主代理       | 强       | 计划、根因判断、最终验收           |
| 执行型子代理 | 弱/中    | 受限 diff、命令输出、execution log |
| 审计型子代理 | 强且独立 | 只读 findings、证据行、风险等级    |

执行者不得兼任最终审计；复杂根因由主代理最终裁决。完整规则见 `SKILL.md` 的“模型分层策略”。
...
- 新增主代理、执行型子代理、诊断协作者和审计型子代理的模型分层契约
- 明确弱/中模型只处理边界明确的执行任务，复杂根因与安全审计升级到强模型
- 要求审计在工作树冻结后独立进行，并由主代理保留最终验收和完成声明
