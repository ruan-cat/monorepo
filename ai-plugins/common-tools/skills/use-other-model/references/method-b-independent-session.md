...
方案 B 是“如何启动独立会话”的执行路径，不代表必须使用最强模型。默认把它当作**执行型子代理**：使用弱/中模型处理明确 diff、批量操作和可复现验证，并通过任务封包限制范围。

- 复杂认证、代理、运行时根因仍由主代理负责；必要时只委托强模型做独立证据采集和假设挑战。
- 敏感信息、方案 pros/cons 和最终 diff 复核应在工作树冻结后交给独立强审计角色，不能由执行会话自审。
- 无论子会话模型强弱，主代理都必须重新查看 diff、原始输出和验证结果后才能验收。
