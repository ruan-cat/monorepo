...
- `agent-progress.md` / `agent-findings.md` 已通过位置门禁：只存在于具体 active/archive change 根目录；发现过错位文件时已经警告、迁移或合并并回读确认。
...
- 工作区没有错位的 `agent-progress.md` / `agent-findings.md`；若存在，已迁移到唯一 active/archive change 或明确阻塞等待用户归属决策。
