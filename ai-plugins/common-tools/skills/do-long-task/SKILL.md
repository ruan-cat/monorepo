...
name: do-long-task
description: >-
  在需要把复杂开发、修复、重构或多步变更拆成可恢复 checkpoint 并持续推进时使用，
  尤其是 OpenSpec 长任务、上下文压缩后的断点续跑、测试失败后的反复修复，
  以及必须把进度、发现和失败写入文件而不能依赖聊天记忆的场景；
  当用户要求生成 Claude Code `/goal`、Codex `/goal`、长任务执行提示词或
  1500 字以内的 do-long-task 提示词时也使用本技能，但只生成提示词不执行任务；
  不要在单文件改动、一次性问答、纯方案讨论、简单格式化、写更新日志
  或其他短任务里使用。
user-invocable: true
metadata:
  version: "1.3.1"
