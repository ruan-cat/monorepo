...
⚠️ **init-prettier-git-hooks 3.0.0 破坏性纠偏**:

- 技能继续由 AI 审计目标项目后逐文件定点合并；不引入迁移 CLI 或运行时代码，并新增 `references/` 保存错误历史与验证剧本。
- `prettier-plugin-lint-md` 必须精确锁定 `1.0.1`，同时核验依赖声明、lockfile 与运行时解析版本；禁止 `^1.0.1` 漂移。
- Prettier 顶层必须使用 `plugins: ["prettier-plugin-lint-md"]`。v2 的对象插件和仅 Markdown override 方案已废弃。
- experimental CLI 与 lint-staged 默认只保留 `--no-parallel`；显式 `--plugin` 仅用于解析故障的诊断 A/B，不是更健壮的生产命令。
- AI 必须保护已有改动；依赖安装、Hook 安装和 Git 行尾归一化等副作用须先获得用户明确授权。

⚠️ **v8.3.2 init-ai-md 模板与报告规范**:

- `init-ai-md` 新增 `11.文档读取策略.md` 模板，要求首次只读标题结构，再按任务读取相关章节。
- 结构化文件按顶层键和相关字段渐进式读取，更新文档时使用精准替换，避免全文读取后整体重写。
- `03.报告编写规范.md` 新增 agent 工具与 AI 模型说明要求。
...
- `init-ai-md` 新增 `09.Karpathy Guidelines.md` 记忆模板。
- 新增「编码前思考、简洁优先、精准修改与目标驱动执行」记忆项。
- 模板整合 Karpathy 编码行为准则，并补充 AI agent 的假设暴露、最小改动、逐步验证和 diff 自检规则。
- 版本号与 marketplace 主版本同步至 `6.9.0`。
...
- 新增 `do-long-task`：面向 OpenSpec 长任务、上下文恢复和测试失败重试的可恢复执行纪律。
- 新增 `AGENT_LONGTASK.md` 与触发评估样例，明确唯一任务源、进度日志、失败记忆和验证后完成规则。
- 版本号与 marketplace 主版本同步至 `6.6.0`。
