...
- **init-linux-cloud-agent-env**：新增技能（`metadata.version` 0.1.0），提供云 linux agent 环境初始化（MCP 客户端与配置、gh/vercel cli）与基于 github pr 的云任务执行规范；`skill-registry.json` 已同步收录。
...
- **release-ai-plugins**：`metadata.version` `2.1.0` -> `2.1.1`。
- release-ai-plugins 技能补充 -NewSkill 强制 patch 升级 metadata.version 的行为说明与规避路径
...
- `common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `10.17.0`。
...
- `common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `10.16.0`。
...
- `common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `10.15.1`。
...
- `common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `10.15.0`。
...
- **init-linux-cloud-agent-env**：`metadata.version` `0.1.0` -> `0.1.1`。
- 补充云 Linux agent 平台实测参考文档
- 根级 Claude / Cursor marketplace 与 `common-tools` / `dev-skills` 的六份三平台 `plugin.json` 版本统一提升至 `10.14.1`。

## [10.14.0] - 2026-09-03

### Added

- **init-linux-cloud-agent-env**：新增技能，`metadata.version` 保持 `0.1.0`。
- 新增 init-linux-cloud-agent-env：云 linux agent 环境初始化与基于 github pr 的云任务执行规范，skill-registry.json 同步收录
- 根级 Claude / Cursor marketplace 与 `common-tools` / `dev-skills` 的六份三平台 `plugin.json` 版本统一提升至 `10.14.0`。

## [10.13.0] - 2026-09-01

### Changed

- **add-favicon**：`metadata.version` `1.0.0` -> `1.1.0`。
...
- **init-prettier-git-hooks**：`metadata.version` `3.2.0` -> `4.0.0`。
- init-prettier-git-hooks 4.0.0：云端 PR Prettier 工作流模板改为无条件安装依赖。
- 根级 Claude / Cursor marketplace 与 `common-tools` / `dev-skills` 的六份三平台 `plugin.json` 版本统一提升至 `10.11.0`。

## [10.10.0] - 2026-09-01

### Changed

- `common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `10.10.0`。
...
- `common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `10.6.3`。
...
- `common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `10.6.1`。
...
- `common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `10.6.0`。
...
- **init-prettier-git-hooks**：`metadata.version` `1.1.4` -> `2.0.0`。
- 将 Prettier Git Hooks 升级为 AI 审计式迁移流程，固定 lint-md 对象插件与串行 experimental CLI 规则
- 根级 Claude / Cursor marketplace 与 `common-tools` / `dev-skills` 的六份三平台 `plugin.json` 版本统一提升至 `9.0.1`。

## [9.0.0] - 2026-08-10

### Changed

`common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `9.0.0`。
...
`common-tools` 技能树本身无内容变更，插件主版本随发布链路同步至 `8.3.6`。

- 根级 Claude / Cursor marketplace 与 `common-tools` / `dev-skills` 的六份三平台 `plugin.json` 版本统一提升至 `8.3.6`。
...
- 本次 `common-tools` 技能树无内容变更；功能更新集中在 `dev-skills` 的 **init-release-base-relizy-and-bumpp** 技能。
...
- 本次 `common-tools` 技能树无内容变更；功能更新集中在 `dev-skills` 的 **use-vercel-deploy-in-monorepo** 新技能。
...
- **git-commit**（`metadata.version` `0.3.6` -> `0.4.0`，重大重构）：
- 步骤 6 的"暂不执行提交"导致 AI 只写文本不执行命令——改为明确"不要在此步骤结束流程"，强制闭环。
- Co-authored-by 在 WorkBuddy 环境静默跳过——改为必输检测日志（客户端/模型/结论），即使不追加也告知用户。
- emoji/type 获取改为远程 raw 优先 + 本地 `references/commit-types.ts` fallback，消除单点故障。
- 步骤 4 敏感信息检查增加"通过后直接继续"，减少不必要的停顿。
- 步骤 9 末尾增加"🛑 不要推送"约束，防御 AI 自动推送。
- 破坏性变更增加主动评估矩阵（6 维判断表格），从等待关键词改为每次提交前主动评估。

### Added

- **git-commit** 新增提交类型选择指南章节：
- 核心/辅助/特殊 3 级分类 + 边界判断流程 + 类型选择禁忌（18 个 type 的适用场景与边界说明）。
- skill 本地 `references/` 目录新增 7 个参考文件（提交拆分示例、暂存区说明、平台命令、黑名单、输入确认、破坏性示例、commit 类型数据），采用渐进式加载模式。
- 提交信息语言选择规则：默认中文，检查上游仓库使用英文时自动切换。

### Removed

- **git-commit** 移除"来源说明"（技能来源翻译信息，对执行无价值）。
- 主 SKILL.md 冗余段落：平台命令参考、优先处理暂存区、分门别类详细示例——均已外迁到 `references/`。
- 步骤 6 重复的 3 个"不得猜测/停止"警告合并为 1 个。

### Changed

- 根级 marketplace 与 `common-tools` / `dev-skills` 双平台 `plugin.json` 版本统一提升至 `7.0.0`。
...
- **record-bug-fix-memory 模板**（`template-version` 新增，值为 `2.0.0`，重大架构升级）：
  - 从旧版单层架构升级为**双层存储架构**：SKILL.md 只放流程指导 + 摘要索引，详细案例拆为独立 `YYYY-MM-DD-{slug}.md` 文件。
  - 新增 `metadata.template-version: "2.0.0"` 字段，支持版本检测与升级判断。
  - 所有章节添加编号前缀（`## 1. 概述` → `## 14. 本仓库落点覆盖`），提升可引用性。
  - 新增「存储架构」章节（§2），含目录结构、案例文件命名规范、必填结构、摘要索引格式。
  - 新增「案例索引」章节（§13），初始为空，随项目积累补充。
  - 新增「本仓库落点覆盖」章节（§14），明确默认落点为技能目录。
  - 重写「写到哪里」章节（§5），改为四级优先级：独立案例 > 摘要索引 > Memorix > 根级 AI 记忆。
  - 重写「记录流程」章节（§6），新增 Memorix 工具可用性核对步骤。
  - 移除旧版「记录什么」「记忆模板」「仓库级经验库」章节，功能由双层架构替代。
  - 「常见错误」新增两条：Memorix 可用性假设、事故正文误写入 SKILL.md。
- **init-ai-md**（`metadata.version` `0.16.0` -> `0.17.0`）：
  - 步骤 5 标题从「初始化本地技能」改为「初始化或升级本地技能」，支持升级流程。
  - 新增旧版检测逻辑：读取 `template-version` 字段，区分 `[可部署]`、`[需升级]`、`[需升级 + 需拆分]`、`[已是最新]` 四种状态。
  - 新增「情况 B：升级骨架」和「情况 C：升级 + 拆分」两种升级路径。
  - 步骤 6 新增 `record-bug-fix-memory` 技能表条目的特殊格式要求（双层存储架构说明）。
  - 「内置技能部署详细说明」章节全面重写，补充双层架构描述、部署注意事项、技能表条目要求。
- 根级 marketplace 与 `common-tools` / `dev-skills` 双平台 `plugin.json` 版本统一提升至 `6.5.0`。
...
- 版本号与根级 marketplace、`common-tools` / `dev-skills` 双平台 `plugin.json` 统一同步至 `6.0.0`。
- 本次 `common-tools` 技能树无内容变更；重大破坏性变更集中在 `dev-skills` 的 **init-release-base-relizy-and-bumpp**（`metadata.version` `2.0.0` → `3.0.0`）。
...
- 版本号与 marketplace 主版本同步至 `4.3.0`（本次 `common-tools` 技能树无内容变更；`dev-skills` 中 **init-release-base-relizy-and-bumpp** 的 `release.yaml` 模板增强见该插件 CHANGELOG）。
...
- 版本号与 marketplace 主版本同步至 `3.2.0`（本次 `common-tools` 技能树无内容变更，与 `dev-skills` 发版保持一致）。
...
- **📝 技能增强**: `init-ai-md` - 新增技能表管理和内置技能部署能力 （v0.15.0 → v0.16.0）
  ...
  - **新增内置技能模板** `record-bug-fix-memory/SKILL.md`：从两个实际项目（01s-11comm、eams-frontend-monorepo）中提取通用化的 bug 修复记忆技能模板，去除项目特有事故记录，保留框架结构供新项目逐步积累
  - **新增文档章节**：技能表管理详细说明、内置技能部署详细说明、内置技能模板规范、场景 4/5 执行示例
  - **执行流程调整**：原步骤 5（同步其他 AI 记忆文件）改为步骤 7，新插入步骤 5（初始化本地技能）和步骤 6（生成/更新技能表）
...
- **🐞 技能修复**: `git-commit` - 修复技能执行顺序问题 （v0.3.0 → v0.3.1）
  ...
  - **修复方案**：调整执行顺序
    ...
    - 步骤 7：验证代码
    - 步骤 8：获取 Co-authored-by → 执行提交 → 删除临时文件 → 验证提交成功
  - **保留细节**：恢复了对 Cursor IDE Shell 工具编码问题的说明
...
- **🔧 技能迭代**: `init-claude-code-statusline` - 优化状态栏布局为两行显示 （v0.14.0 → v0.14.1）
  ...
  - **优化方案**：将状态栏拆分为两行显示
    - 第一行：仅显示目录路径
    - 第二行：显示 Git 分支、模型名称、版本号、上下文窗口使用情况
    - 第三行：成本和使用信息（条件显示）
  - **用户体验提升**：解决了目录地址过长和模型信息过长导致显示不清的问题
...
- **版本**: v0.0.3 → v0.2.0
- **变更思路**: 防止 AI 操作在代码库提交上的过度膨胀，约束操作粒度，增加精确命中用户意图的行为检查锚点（暂存区判别与四维拆解）。
