...
## 01 处理 claude code 钩子错误 `task-complete-notifier.sh` 缺失 CLAUDE_PROJECT_DIR 的报错
...
## 02 处理 claude code 通知钩子每次都重复运行两次的故障
...
## 03 检查无头模式下 gemini 的输入数据，和输出结果，并排查为什么 gemini 每次总结时，总结文本都只有五个字：`任务已完成`
...
## 05 ~~处理 `● Stop hook prevented continuation` 故障提示~~ ✅ 已解决
...
## 08 修复 claude code 钩子无法阅读有效的上下文日志的故障
...
## 09 修复 claude code 钩子无法阅读有效的上下文日志的故障，每次阅读的都是固定的`任务处理完成`文本
...
## 10 修复错误 `● Stop hook failed: The operation was aborted`
...
## 11 升级 claude code 插件的版本，编写更新日志
...
## 12 处理 claude code 关于 `hooks/hooks.json` 的导入路径错误
...
## 13 claude code 插件的 `Notification` 钩子，增加 `@ruan-cat/claude-notifier` 包提供的通知类型
...
## 15 针对 Stop hooks 处理 `● Stop hook failed: The operation was aborted` 的故障
...
### 15.04 关注分析 `claude-notifier check-and-notify` 的逻辑
...
### 15.05 由 `check-and-notify.ts` 提供的报错日志
...
## 18 处理 claude code 插件的 Stop 钩子没有触发任何一个 `claude-notifier task-complete` 通知的故障
...
### 19.02 细化增强 `claude-code-marketplace\common-tools\skills\openspec\SKILL.md` 文档
...
## 20 处理 claude code Stop hook 的故障
...
## 21 新建一款专门初始化当前项目的 claude code 状态栏配置的 claude code skills
...
重点说明清楚其要使用 Conventional Commits 规范的感叹号来编写破坏性变更。

每当用户提及到【破坏性变更】这个关键词的时候，就必须使用感叹号来编写破坏性变更。其感叹号必须紧跟在 type 后面，且不留空格。必须要满足严格的 Conventional Commits 规范。

### 优先针对 git 暂存区的文件做提交

让该技能优先针对 git 暂存区的文件做提交。而不是针对整个工作树的文件做提交。

如果 git 暂存区没有文件，则针对工作树的文件做提交。

### 学会分门别类的针对性拆分多个提交

在使用该技能时，【分门别类】是一个高频词。这样要求你务必要认真分析对话的上下文内容，理解透彻本次修改的内容。然后针对性的拆分多个提交。

特别是 git 工作树文件数目很多的时候，务必要针对性的拆分多个提交。
