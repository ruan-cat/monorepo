...
- 统一 Lucide 风格。
- 每个站点使用不同图标，按项目语义区分。
- 不再使用圆形底色，直接展示透明背景上的 icon。
- 每个图标使用 1 个主色，避免浏览器标签页出现黑乎乎的一块背景。
...
- 浏览器标签页中只呈现线性 icon，不再出现外层色块。
- 站点之间通过 icon 语义和单色描边区分。
- Vercel 部署工具站点保留深色云上传图标，但深色只用于线条，不再作为背景块。
