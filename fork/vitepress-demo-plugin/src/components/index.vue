...
import { ref, inject, watch, nextTick, computed, Ref, onUnmounted, watchEffect, onMounted } from "vue";
...
const currentFiles = computed<Record<string, { code: string; filename: string }>>(() => {
...
async function updatetDisplayCode() {
	displayCode.value = await codeToHtml(currentCode.value || "", {
		lang: currentFiles.value[activeFile.value]?.filename.split(".").pop() || fileType.value,
		themes: {
...
<template>
	<div :class="[ns.e('container')]">
		...
		<div :class="ns.bem('fold')" v-if="!isCodeFold" @click="setCodeFold(true)"><FoldIcon />收起代码</div>
	</div>
</template>
