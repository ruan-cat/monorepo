 ...
 * @returns
 */
export const transformPreview = (md: MarkdownIt, token: Token, mdFile: any, config?: VitepressDemoBoxConfig) => {
	const {
		demoDir,
		tab = {},
	...
	if (vuePathRegexValue?.[1]) {
		componentProps.vue = path.join(dirPath, vuePathRegexValue[1]).replace(/\\/g, "/");
	}

	if (htmlPathRegexValue?.[1]) {
		componentProps.html = path.join(dirPath, htmlPathRegexValue[1]).replace(/\\/g, "/");
	}
	if (reactPathRegexValue?.[1]) {
		componentProps.react = path.join(dirPath, reactPathRegexValue[1]).replace(/\\/g, "/");
	}
	...
	const componentVuePath = componentProps.vue
		? path.resolve(demoDir || path.dirname(mdFile.path), vuePathRegexValue?.[1] || ".").replace(/\\/g, "/")
		: "";
	const componentHtmlPath = componentProps.html
		? path.resolve(demoDir || path.dirname(mdFile.path), htmlPathRegexValue?.[1] || ".").replace(/\\/g, "/")
		: "";
	const componentReactPath = componentProps.react
		? path.resolve(demoDir || path.dirname(mdFile.path), reactPathRegexValue?.[1] || ".").replace(/\\/g, "/")
		: "";

	// 组件名
	// eslint-disable-next-line prefer-destructuring
	const absolutePath = path
		.resolve(dirPath, componentProps.vue || componentProps.react || componentProps.html || ".")
		.replace(/\\/g, "/");
	...
	if (componentProps.react) {
		injectComponentImportScript(mdFile, "react", "{ createElement as reactCreateElement }");
		injectComponentImportScript(mdFile, "react-dom/client", "{ createRoot as reactCreateRoot }");
		injectComponentImportScript(mdFile, componentReactPath, reactComponentName, "dynamicImport");
	}
	...
	// 组件代码，动态引入以便实时更新
	const htmlCodeTempVariable = componentProps.html ? `TempCodeHtml${componentName}` : `''`;
	const reactCodeTempVariable = componentProps.react ? `TempCodeReact${componentName}` : `''`;
	const vueCodeTempVariable = componentProps.vue ? `TempCodeVue${componentName}` : `''`;
		...
		if (value) {
			try {
				...
				for (const file in files[key as keyof typeof files]) {
					...
					if (filePath) {
						const absPath = path.resolve(demoDir || path.dirname(mdFile.path), filePath || ".").replace(/\\/g, "/");
