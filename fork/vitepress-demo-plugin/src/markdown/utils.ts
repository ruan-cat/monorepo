// <demo></demo> or <demo />
export const demoReg = [/<demo(\s|\n)((.|\n)*)><\/demo>/, /<demo(\s|\n)((.|\n)*)\/>/];

const scriptLangTsReg = /<\s*script[^>]*\blang=['"]ts['"][^>]*/;
const scriptSetupReg = /<\s*script[^>]*\bsetup\b[^>]*/;
const scriptSetupCommonReg = /<\s*script\s+(setup|lang='ts'|lang="ts")?\s*(setup|lang='ts'|lang="ts")?\s*>/;
...
export const injectComponentImportScript = (
	...
	const scriptsSetupIndex = scriptsCode.findIndex((script: any) => {
		if (scriptSetupReg.test(script.tagOpen) || scriptLangTsReg.test(script.tagOpen)) return true;
		return false;
	});
    ...
    `.trim();
		...
		importCode = name ? `import ${componentName} from '${path}'` : `import '${path}'`;
	}
	...
	if (scriptsSetupIndex === -1) {
		...
		if (oldScriptsSetup.content.includes(path) && (!name || oldScriptsSetup.content.includes(componentName))) {
			...
			scriptsCode[0].content = scriptsCode[0].content.replace(scriptSetupCommonReg, scriptCodeBlock);
...
export const composeComponentName = (path: string) => {
	...
	return (
		"Temp" + btoa(encodeURIComponent(componentList.join("-").split(".").slice(0, -1).join("."))).replace(/=/g, "Equal")
	);
};
