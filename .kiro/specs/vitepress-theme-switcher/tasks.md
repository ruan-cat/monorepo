...
- [ ] 1. 搭建项目结构和依赖
  - [ ] 1.1 在 package.json 中添加 @voidzero-dev/vitepress-theme 和 fast-check 依赖
    - 将 `@voidzero-dev/vitepress-theme` 添加到 dependencies
    - 将 `fast-check` 添加到 devDependencies 用于属性测试
    - _Requirements: 6.1, 6.2, 6.3_
  - [ ] 1.2 创建 theme-switcher 目录结构
    - 创建 `src/theme-switcher/` 目录
    - 创建子目录：`components/`、`adapters/`、`__tests__/`
    - _Requirements: 3.1_

- [ ] 2. 实现 ThemeRegistry 核心模块
  - [ ] 2.1 在 types 中创建 ThemeDefinition 和 ThemeRegistry 接口
  ...
  - [ ] 2.2 实现 ThemeRegistry 类
    - 实现基于 Map 的主题存储
    - 实现支持覆盖的 register 方法
    - 实现 get、getAll、has、size 方法
    - _Requirements: 3.1, 3.4, 3.5_
  - [ ] 2.3 编写 ThemeRegistry 注册的属性测试
    - **Property 6: 主题注册接受性**
    - **Validates: Requirements 3.1**
  - [ ] 2.4 编写 ThemeRegistry 覆盖行为的属性测试
    - **Property 8: 主题覆盖行为**
    - **Validates: Requirements 3.4**
  - [ ] 2.5 编写 ThemeRegistry 查找正确性的属性测试
    - **Property 9: 主题查找正确性**
    - **Validates: Requirements 3.5**

- [ ] 3. 实现 ThemePersistence 模块
  - [ ] 3.1 创建 ThemePersistence 接口和实现
...
- [ ] 6. 实现 ThemeManager 核心模块
  - [ ] 6.1 创建 ThemeManager 接口和实现
  ...
  - [ ] 6.3 编写 CSS 动态更新的属性测试
    - **Property 12: CSS 动态更新**
    - **Validates: Requirements 5.2**

- [ ] 7. 实现 ThemeSwitcherConfig 并扩展 ExtraConfig
  - [ ] 7.1 在 types.ts 中添加 ThemeSwitcherConfig 接口
...
- [ ] 9. 实现 ThemeSwitcherButton Vue 组件
  - [ ] 9.1 创建 ThemeSwitcherButton.vue 组件
    - 实现带主题列表的下拉菜单
    - 实现主题选择处理器
    - 样式与 VitePress 导航栏匹配
    - _Requirements: 1.1, 1.3_
  - [ ] 9.2 创建 useThemeSwitcher 组合式函数
    - 提供响应式主题状态
    - 暴露 switchTheme 和 getAvailableThemes 方法
    - 处理初始化和持久化
    - _Requirements: 1.2, 1.4, 5.3_

- [ ] 10. 将主题切换器集成到现有 theme.ts
  - [ ] 10.1 修改 defineRuancatPresetTheme 以支持主题切换器
    - 向 DefineRuancatPresetThemeParams 添加可选的 themeSwitcher 参数
    - 有条件地注册 ThemeSwitcherButton 组件
    - 保留现有的 enhanceApp 行为
    - _Requirements: 2.2, 2.4_
  - [ ] 10.2 编写向后兼容性的属性测试 - defineRuancatPresetTheme
    - **Property 4: 向后兼容性 - defineRuancatPresetTheme**
    - **Validates: Requirements 2.2**

- [ ] 11. 将主题切换器集成到现有 config.mts
  - [ ] 11.1 修改 setUserConfig 以处理主题切换器配置
  ...
  - [ ] 11.2 编写向后兼容性的属性测试 - setUserConfig
    - **Property 5: 向后兼容性 - setUserConfig**
    - **Validates: Requirements 2.3**

- [ ] 12. 实现 VoidZero 主题清理逻辑
  - [ ] 12.1 向 VoidZero 适配器添加清理函数
    - 从文档中移除 VoidZero 特定的样式元素
    - 清理任何 VoidZero 特定的 CSS 变量
    - _Requirements: 6.4_
  - [ ] 12.2 编写 VoidZero 主题清理的属性测试
