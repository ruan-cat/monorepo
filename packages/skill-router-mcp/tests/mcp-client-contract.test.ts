...
describe("MCP Web Standard Streamable HTTP contract", () => {
	test("initializes and discovers tools through the SDK client", async () => {
		...
		try {
			...
			expect(client.getServerVersion()).toEqual({ name: "skill-router-mcp", version: "0.1.0" });
	...
	test("rejects oversized request and response bodies without leaking runtime details", async () => {
