...
- 1. 修复 `relizy-runner` 在 `changelog --dry-run --yes` 场景下会把 `--yes` 透传给上游 `relizy` 并触发非法参数报错的问题；现在 `changelog` 子命令下会兼容接受并忽略该参数。 ([`7c8b64c`](https://github.com/ruan-cat/monorepo/commit/7c8b64c7bf229e6453b4704f1d9cb6742b94e299))
...
- 1. 新增 CLI 命令行工具，通过标准 `bin` 字段提供 `ruan-cat-utils` 和 `move-vercel-output-to-root` 两个可执行命令 ([`abf9c57`](https://github.com/ruan-cat/monorepo/commit/abf9c577bc4a1663894cf455319820598fe68961))
...
- 1. 新增 `findMonorepoRoot()` 公共函数，并通过 `@ruan-cat/utils/monorepo` 对外提供统一的 monorepo 根目录定位入口。该函数会从当前工作目录开始向上查找 `pnpm-workspace.yaml`，适用于脚本在 monorepo 子包内执行时的根目录寻址场景。 ([`16cada1`](https://github.com/ruan-cat/monorepo/commit/16cada15096f374829759755261018dd54c36adc))
