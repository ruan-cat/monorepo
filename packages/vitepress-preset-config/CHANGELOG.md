...
- Updated dependencies [[`d0de2f5`](https://github.com/ruan-cat/monorepo/commit/d0de2f59d0c9e4f9c27e90656abc8abba8ce0e88)]:
...
- 1. 重构 `copy-claude-files.ts` 内部的 monorepo 根目录定位逻辑，移除包内重复维护的 `findMonorepoRoot()` 私有实现，改为直接复用 `@ruan-cat/utils/monorepo` 提供的公共函数。 ([`16cada1`](https://github.com/ruan-cat/monorepo/commit/16cada15096f374829759755261018dd54c36adc))
...
- 1. 移除了 `vitepress-theme-teek/theme-chalk/tk-sidebar.css` 的侧边栏样式导入，升级后侧边栏标题将不再沿用原先的加粗视觉效果。 ([`eebe3a4`](https://github.com/ruan-cat/monorepo/commit/eebe3a4cfd3cca02055da21e8160e2b659105909))
