...
v2.0 将 `vercel` 从内部依赖改为 peer dependency。使用方项目需要自己安装并维护 Vercel CLI 版本。

```bash
pnpm add -D @ruan-cat/vercel-deploy-tool@latest vercel@latest
```

迁移要点：

1. `@ruan-cat/vercel-deploy-tool` 不再内置 `vercel` 包。
2. 项目内必须显式安装 `vercel`，最低版本为 `47.2.2`。
3. CI 中建议检查 `pnpm exec vercel --version`，避免锁文件继续使用旧版 CLI。
...
```bash
# 更新部署工具，并显式安装 Vercel CLI peer 依赖
pnpm add -D @ruan-cat/vercel-deploy-tool@latest vercel@latest
```
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Check Vercel CLI
        run: pnpm exec vercel --version
...
- [ ] 已更新到 v1.0.0+
- [ ] 已显式安装 `vercel@latest` 或 `vercel>=47.2.2`
