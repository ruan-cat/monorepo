...
- 1. 完善钻头文档 `smallalice-docs-ai-nitro-api.ruan-cat.com` 的域名配置说明，明确其部署于 Vercel，并对应 `SmallAliceWeb/packages/ai-rag-api` 的 AI RAG Nitro 服务端接口入口。 ([#129](https://github.com/ruan-cat/monorepo/pull/129))
  2. 补充该服务承载 Markdown 文档知识源接入、PostgreSQL/pgvector 与 OpenAI 混合检索、带来源引用的流式 RAG 问答、知识库同步与检索评测等能力的用途说明。

## 2.6.0

### Minor Changes

- 1. 为 `11comm` 项目域名配置新增 `projectAlias` 字段，用于标注 Vercel 项目别名。 ([`1345b28`](https://github.com/ruan-cat/monorepo/commit/1345b28825c9b52c0e54ba34dd2c71a60b593dbc))
...
- 1. 新增 `DeployPlatform` 类型（`"cloudflare" | "vercel"`），并在 `ProjectLikeDomain` 接口中添加 `deployPlatform` 可选字段，用于标注域名的部署平台。 ([`51f2b20`](https://github.com/ruan-cat/monorepo/commit/51f2b20b09a9d315fdbc90fe2820caff0cb7ff24))
