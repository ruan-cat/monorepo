...
2. 从正文删除的内容，必须先原文迁移到 `.agents/skills/skill-hardening-from-incidents/reference/` 下最合适的主题文件，再压缩正文；不能先删后补，也不能只留一句无法恢复的摘要。
3. 每次迁移都要在 [`reference/README.md`](reference/README.md) 登记“来源章节、目标文件、迁移原因、验证状态”。参考文件是长期保留层，不是临时草稿。
4. 参考文件只能追加或定向修订；如果规则过时，保留旧版本及废弃原因，不得静默覆盖仍可能有复用价值的证据。
5. 主文件只保留稳定入口、强制边界和快速检查；细节通过参考文件渐进披露。压缩字数不能作为删除知识的理由。
6. 完成前检查孤立标题、失效链接、未登记的删除块和重复/冲突规则；任何无法解释的减少都视为未完成。

完整契约见 [`reference/retention-contract.md`](reference/retention-contract.md)。

## 核心执行流程

1. **界定写集**：明确本轮允许修改的 skill、参考文件、根级 AI 记忆和（如适用）对外分发入口；不确定时先问用户。
2. **建立基线**：在编辑前运行压力场景或静态断言，记录当前缺口和可复现的失败输出。
3. **读取证据**：核对用户材料、当前 skill、相关 diff、根级 AI 记忆、同类事故和发布/同步元数据；证据不足只记录待确认项。
4. **提炼规则**：按“现象 → 根因 → 错误诱因 → 未来规则 → 验证方式”编写，禁止把事故流水账直接贴进正文。
5. **迁移与编辑**：先完成参考文件迁移和索引登记，再用最小补丁更新入口；不得顺手格式化无关文件。
6. **独立验证**：主代理亲自查看最终 diff，执行 frontmatter、链接、路径污染、内容保留和关键命令检查；不能只相信子代理报告。
...
- 被正文移除的内容均已进入 `reference/`、登记来源并通过保留检查。
