...
- `2026-08-11-vitepress-node24-pnpm-entrypoint.md` — Node.js 24 CI 中 VitePress 宽 barrel 间接加载 `consola`，且 utils 多组 tsup 配置并行清理 `dist` 会删除声明；修复方式为真实窄子路径出口、一次性清理构建目录、删除全局 patch，并用入口测试确定 `consola`、`tinyglobby` 的最小 ESM 内联边界。
