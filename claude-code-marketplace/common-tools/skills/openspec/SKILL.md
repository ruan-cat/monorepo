...
1. 按照严格顺序来执行 `tasks.md` 任务清单所列举的任务。
   > 在执行 openspec 任务的时候，请你严格的按照顺序，从上往下的执行任务。不允许出现任务跳过的情况。
2. 及时更新 `tasks.md` 任务清单所列举的任务。
   > 在执行长任务时，你会大批量的完成很多任务，但是你很容易忘记及时的去更新 `tasks.md` 任务清单。请你务必要及时的更新任务清单，避免出现多次运行重复完成任务的情况。
3. 主动使用多个并行的子代理拆分长任务。
   > 在实际执行 openspec 的 `tasks.md` 任务时，当你意识到这是一个长任务时，你应该主动的拆分出多个并行的子任务。用子代理的方式，新建多个并行运行的子任务。以确保主代理的上下文清晰干净，更好的实现单一主代理完成长时间任务的目标。
4. **禁止**编写批处理脚本来批处理代码。
   > 不允许通过编写 python 脚本，javascript 脚本，typescript 脚本的方式，来完成代码文件的批处理任务。
